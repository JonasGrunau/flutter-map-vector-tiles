...
| File | Description |
|------|-------------|
| `vector_tile_layer.dart` | The `VectorTileLayer` widget and its state — the orchestrator (~1500 lines). Owns tile stores, the display-tile map, retained-tile pruning (also release-on-readiness from the publish jobs and the fade tick, since a rebuild needs a camera change the end of a gesture never delivers), the raster job queue, the fade ticker, and the `CustomPainter` that draws tile images then labels. Cache-served tiles fade in only when retained imagery lies beneath them (`_coveredByRetained`) — a fade over the bare background is the zoom-out ring shimmer. A pending source renders provisionally from its nearest cached ancestor, or — zoom-out, no ancestor — from its cached descendants composed into the tile (geometry only). Its label-continuity duties are down to feeding the painter: pinning an outgoing level's cohorts to what was drawn (`_drawnLastFrame`), splitting retained tiles into placement candidates vs ghost-only fallbacks, parking a disposed retained tile's labels (`_ghostLabels`) for one fade duration, and bumping `_labelGeneration` whenever the camera or candidate set changes. The painter picks a changed generation up on its throttled cadence; ticker-only repaints keep the same generation and create no new placement debt — the fades and the placement decision itself live in the painter |
