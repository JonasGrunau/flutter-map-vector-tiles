...
| `label_continuity.dart` | Matches labels across a zoom level change, so only genuine appearances and disappearances animate: `labelContinuityKey` (`(layer, text, icon)` — deliberately position-free), `coveringLabelKeys` (what the overlapping retained tiles are showing), `partitionCarriedOver` (carried-over prefix first, plus its length — only the rest fades in), `drawnLabels` (a cohort filtered to what the last frame actually drew — retention pins outgoing cohorts to it so they can keep labels but never introduce them, and the fade-out starts from it) and `orphanedLabels` (what the arriving level does not replace — what fades out). Pure; the widget only supplies the cohorts |
...
- `test/label_zoom_gating_test.dart` — per-frame layer zoom-range gate, the
  ...
  collision tiebreak
- `test/label_continuity_test.dart` — cross-zoom label carry-over and
  fade-out: key identity, cohort partitioning, orphan detection, the
  two-copies-collide case a zoom crossing produces, the placement of
  a departing label (keeps the space it held, deduplicated like any
  other), and the outgoing-cohort pin (`drawnLabels` — a candidate a
  zoom-cut label was suppressing must not pop in mid-transition)
