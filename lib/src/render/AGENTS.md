...
| `label_continuity.dart` | Label continuity primitives: position-free identity, per-sitting fade state, throttled collision placement, remembered placement choices and outgoing-cohort filtering. `PlacementThrottle` receives a generation that changes with camera/candidate input, waits until the fade cadence permits a pass, and reports `deferred` only until that generation is placed; unchanged animation repaints never create another debt. Pure and clock-agnostic; the painter drives it. |
