...
| `tile_result_cache.dart` | `TileResultCache` + `TileResult` — process-wide LRU of *finished* display tiles (rasterized `ui.Image` + `SymbolInstance` list + `renderedWith`), one cache per render signature. The cache owns master images and disposes them on eviction; consumers take `clone()`s. Bounded by `rasterCacheMaxBytes` (GPU texture bytes). Makes zoom-level re-crossings and same-style reopens paint without re-rendering |
