...
| File | Description |
|------|-------------|
| `tile_processor.dart` | `PrepareInput` (bytes + tile key + the per-layer properties the theme actually references) and `prepareTileSync(PrepareInput)` — the pure decode + trim step that runs on the worker |
