...
Color? parseCssColor(String input) {
  ...
  if (s.startsWith('hsl')) return _parseHsl(s);
  return _named[s];
}

Color? _parseHex(String hex) {
  int? channel(String s) => int.tryParse(s, radix: 16);
  switch (hex.length) {
    case 3:
    case 4:
      final r = channel(hex[0] * 2);
      final g = channel(hex[1] * 2);
      final b = channel(hex[2] * 2);
      final a = hex.length == 4 ? channel(hex[3] * 2) : 255;
      if (r == null || g == null || b == null || a == null) return null;
      return Color.fromARGB(a, r, g, b);
    case 6:
    case 8:
      final r = channel(hex.substring(0, 2));
      final g = channel(hex.substring(2, 4));
      final b = channel(hex.substring(4, 6));
      final a = hex.length == 8 ? channel(hex.substring(6, 8)) : 255;
      if (r == null || g == null || b == null || a == null) return null;
      return Color.fromARGB(a, r, g, b);
  }
  return null;
...
double? _number(String s, {double percentScale = 1}) {
  if (s.endsWith('%')) {
    final v = double.tryParse(s.substring(0, s.length - 1));
    return v == null ? null : v / 100 * percentScale;
  }
...
Color? _parseRgb(String s) {
  ...
  double? channel(String p) =>
      p.endsWith('%') ? _number(p, percentScale: 255) : _number(p);
...
Color? _parseHsl(String s) {
  final parts = _args(s);
  if (parts == null || parts.length < 3) return null;
  final h = _number(parts[0].replaceAll('deg', ''));
  final sat = _number(parts[1]);
